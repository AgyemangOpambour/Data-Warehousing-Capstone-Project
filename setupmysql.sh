...
mysql --host=172.21.108.7 --port=3306 --user=root --password="uz1X5M79K60woCcNDGbsiEXj" -e "create database sales;
...
mysql --host=172.21.108.7 --port=3306 --user=root --password="uz1X5M79K60woCcNDGbsiEXj" -e "use sales;load data infile '/home/project/sales_olddata.csv' into table sales_data fields terminated BY ','lines terminated BY '\n';"

# Loading data from sales_newdata.csv into sales_data table.This csv consists of data updated with the current timestamp.
mysql --host=172.21.108.7 --port=3306 --user=root --password="uz1X5M79K60woCcNDGbsiEXj" -e "use sales;load data infile '/home/project/sales_newdata.csv' into table sales_data fields terminated BY ',' lines terminated BY '\n'(rowid,product_id,customer_id,price, quantity);"
